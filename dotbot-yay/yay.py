# source: https://github.com/alexwh/dotbot-yay
import sys
import os
import subprocess
import dotbot
...
class Yay(dotbot.Plugin):
    ...
    def _install(self, pkg):
        # new code: check if package already exists
        process = subprocess.Popen('yay -Qi {}'.format(pkg),
                shell=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT)
        out = process.stdout.readlines()[0].decode()
        process.stdout.close()
        if not "error: package" in out and not "was not found\n" in out:
